...
class InvoiceAppGUI:
    ...
    def add_product_dialog(self):
        ...
        for i, field in enumerate(fields):
            ttk.Label(input_frame, text=f"{field}:").grid(row=i, column=0, sticky="w", pady=5)
    ...
    def view_invoice_details(self):
        ...
        ttk.Label(summary_frame, text=f"Tổng tiền: {invoice.total_amount:,.0f} VND", font=("Arial", 12, "bold")).pack(side="right")
    ...
    def create_new_invoice(self):
        ...
        ttk.Label(info_frame, text="Tên khách hàng:").grid(row=0, column=0, sticky="w", pady=5, padx=5)
        customer_name_entry = ttk.Entry(info_frame, width=40)
        ...
        ttk.Label(add_item_frame, text="Sản phẩm:").grid(row=0, column=0, padx=5, pady=5, sticky='w')
        product_var = tk.StringVar()
        product_combo = ttk.Combobox(add_item_frame, textvariable=product_var, width=35, state="readonly")
        ...
        # Ô nhập số lượng
        ttk.Label(add_item_frame, text="Số lượng:").grid(row=0, column=2, padx=5, pady=5, sticky='w')
